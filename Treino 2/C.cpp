...
typedef tree<pair<int,int>, null_type, less_equal<pair<int,int>>, rb_tree_tag, tree_order_statistics_node_update> pds; 
...
void solve(){
    ...
    while(q--){
        cin>>a>>b;
        tree.erase(tree.find_by_order(tree.order_of_key(mp[a])));
        mp[a] = {mp[a].f-1,mp[a].s+b};
        tree.insert(mp[a]);
        cout << tree.order_of_key(mp[1])+1 << endl;
    }
}        
